...
public class GeneradorReportes {
    ...
    public static void generarKardexArticuloPDF(String ruta, String nombreArticulo, List<ReporteKardexDTO> datos) throws Exception {
        try (FileOutputStream fos = new FileOutputStream(ruta)) {
            ...
            tabla.setWidthPercentage(100);
            tabla.setWidths(new float[]{1.5f, 1.5f, 1f, 1.5f, 1.5f, 2f});
            tabla.addCell(crearCeldaEncabezado("Fecha"));
            tabla.addCell(crearCeldaEncabezado("Movimiento"));
            tabla.addCell(crearCeldaEncabezado("Cant."));
            tabla.addCell(crearCeldaEncabezado("Costo Unit."));
            tabla.addCell(crearCeldaEncabezado("Costo Prom."));
            tabla.addCell(crearCeldaEncabezado("Referencia"));

            for (ReporteKardexDTO row : datos) {
                tabla.addCell(row.getFecha().toString());
                tabla.addCell(row.getTipoMovimiento().toUpperCase());
                tabla.addCell(String.valueOf(row.getCantidad()));
